...
def collect_args():
    ...
    parser.add_argument('-tr', '--test_recipient',
                        default=None,
                        help='Only generate a notification for this email address')
    ...
    parser.add_argument('-t', '--template', action='store',
                        help='Name of template to use. Templates to be stored in ./template/',
                        required=True)
...
def create_notification(user_id, user, start_ts, end_ts, tz, zone, node, test_recipient, work_dir, template):
...
# print work_dir + '/' + email
    ...
    if affected_instances > 0:
        render_templates(subject, instances, start_ts, end_ts, tz,
            zone, affected, node, work_dir + '/' + email, template)
        return True
    
    return False
        

def render_templates(subject, instances, start_ts, end_ts, tz, zone, affected, node, filename, template):
...
def get_instances(client, zone=None, inst_status=None, node=None):
    ...
    if node:
        response = client.servers.list(search_opts={'all_tenants': 1, 'host': node})
        for server in response:
            yield server
    #Collect instances for entire availability zone
    #marker - "begin returning servers that appear later in the server list than
    #that represented by this server id
    #There must be a return limit on the number of servers, so we need to keep
    #scanning until the response is null, shifting the marker each time.
    else:
        while True:
            ...
            for instance in response:
                marker = instance.id
                instance_az = instance._info.get('OS-EXT-AZ:availability_zone') or ''
...
#EndGoal
#List of users with instance information
...
def main():
    ...
    work_dir='./outbox/' + datetime.datetime.now().strftime("%y-%m-%d_%H:%M:%S")
    ...
    for uid, user in user_data.iteritems():
        if test_recipient:
            ...
            if user['email'] == test_recipient:
                if create_notification(uid, user, start_ts, end_ts, args.timezone, zone,
                     args.node, test_recipient, work_dir, template):
                    count += 1
        else:
            if create_notification(uid, user, start_ts, end_ts, args.timezone, zone,
                            args.node, test_recipient, work_dir, template):
    ...
    print "\n./send_all_email.py -o " + work_dir + '/' + " [ -p smtp.unimelb.edu.au ] [ -tr send_all_emails@to_this_test_address.com ]"
